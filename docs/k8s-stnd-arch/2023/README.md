...
이에 시장에서 많은 레퍼런스를 가지고 있고, 안정적으로 동작하는 제품들로 구성된 표준 아키텍처가 있어야 한다고 판단되었습니다. 이에 다음과 같이 표준 아키텍처를 구성하였으나, 만약 이미 잘 쓰고 있는 구성 및 제품들이 있다면, 해당 내용이 가장 나에게 좋은 선택지 입니다. 따라서 현재 선정한 제품들은 쿠버네티스를 시작하는 분들은 위한 가이드 또는 새로운 영역을 구성 추가할때 참고할 수 있는 내용이라고 이해해 주시면 좋겠습니다. 
...
하지만 만약 Best Practice적인 성격으로 2023년에 구성을 고민하신다면, 현재의 구성이 큰 도움이 될 것이라고 생각합니다. 부디 이를 통해서 금년도 쿠버네티스로의 진입이 더욱 더 수월하셨으면 합니다.
