...
그럼에도 2026년에는 매우 큰 변화가 있었는데, **OCI의 적극적인 도입**과 **Ingress NGINX의 지원 종료 예정**입니다. 이와 관련한 자세한 내용은 헬름, 하버 그리고 인그레스를 참고하시기 바랍니다.  <br>
이와 같이 쿠버네티스는 단일 제품으로 존재하는 것이 아니라 쿠버네티스 상에서 동작하는 다양한 컨테이너 애플리케이션을 유기적으로 연결하여 다양한 목적으로 사용하게 됩니다. 따라서 이를 위한 성숙한 프로젝트들은 매우 중요한 요소입니다.2025년 10월 28일 을 기준으로 클라우드 네이티브 컴퓨팅 재단(CNCF, Cloud Native Computing Foundation)의 졸업 프로젝트는 총 34개가 발표되었습니다.
